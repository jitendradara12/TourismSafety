...
function labelStyle() {
  return { display: "block", fontSize: 12, color: "#6b7280", marginBottom: 4 } as const;
}
function inputStyle() {
  return { width: "100%", padding: "8px 10px", border: "1px solid #e5e7eb", borderRadius: 6 } as const;
}
function rowStyle() {
...
export default function DigitalIdDemoPage() {
  ...
  return (
    <main style={{ padding: 16 }}>
      <a href="/" style={{ color: "#2563eb", display: "inline-block", marginBottom: 8 }}>&larr; Home</a>
      <h1 style={{ fontSize: 22, fontWeight: 600 }}>Digital Tourist ID — Demo</h1>
      <p style={{ color: "#6b7280", marginTop: 4 }}>Simulated issuance and verification with local hashing and a revocation list demo. No data leaves your browser.</p>

      <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
        <button onClick={() => setMode('generate')} style={{ padding: '6px 10px', borderRadius: 6, background: mode==='generate' ? '#111827' : '#e5e7eb', color: mode==='generate' ? '#fff' : '#111827' }}>Generate</button>
        <button onClick={() => setMode('verify')} style={{ padding: '6px 10px', borderRadius: 6, background: mode==='verify' ? '#111827' : '#e5e7eb', color: mode==='verify' ? '#fff' : '#111827' }}>Verify</button>
        <button onClick={onLoadLocal} style={{ padding: '6px 10px', borderRadius: 6, background: '#f3f4f6' }}>Load last</button>
      </div>
      ...
      <form onSubmit={onSubmit} style={{ marginTop: 12, display: "grid", gap: 12 }}>
        ...
        {error && (
          <div style={{ padding: 10, border: "1px solid #fee2e2", background: "#fef2f2", color: "#7f1d1d", borderRadius: 8 }}>{error}</div>
        )}

        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button disabled={!isValid || submitting} type="submit" style={{ padding: "8px 12px", background: "#16a34a", color: "white", borderRadius: 6, opacity: (!isValid || submitting) ? 0.6 : 1 }}>
            {submitting ? "Anchoring…" : "Generate Digital ID"}
          </button>
          <button type="button" onClick={resetAll} style={{ padding: "8px 12px", background: "#e5e7eb", borderRadius: 6 }}>
            Reset
          </button>
        </div>
      </form>
      ...
      {result && (
        <section style={{ marginTop: 16, border: "1px solid #e5e7eb", borderRadius: 8, background: "#ffffff" }}>
          <div style={{ padding: "10px 12px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 12, color: isRevoked ? '#991b1b' : '#16a34a' }}>{isRevoked ? 'Revoked (demo)' : 'Success'}</span>
          </div>
          <div style={{ padding: 12 }}>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: 'wrap' }}>
              <button onClick={onDownload} style={{ padding: "8px 12px", background: "#dbeafe", color: "#1e40af", borderRadius: 6 }}>Download JSON</button>
              <a href="#" onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(result.did); }} style={{ padding: "8px 12px", background: "#f3f4f6", borderRadius: 6, color: "#111827", textDecoration: "none" }}>Copy DID</a>
              <button onClick={onSaveLocal} style={{ padding: '8px 12px', background: '#fef3c7', color: '#78350f', borderRadius: 6 }}>Save to device</button>
              <button onClick={onShare} style={{ padding: '8px 12px', background: '#e9d5ff', color: '#5b21b6', borderRadius: 6 }}>Share</button>
              <button onClick={onPrintCard} style={{ padding: '8px 12px', background: '#dcfce7', color: '#065f46', borderRadius: 6 }}>Print ID Card</button>
